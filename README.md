...
QQ 单聊和群聊事件不提供引用消息 ID。适配器会在 SQLite 中按会话保留最近 5 分钟的图片；用户发送或引用图片后再发送文字命令时，最近图片会随命令上报给 core。频道事件若提供 `message_reference`，仍会优先按精确消息 ID 查找。消息 ID 图片索引保留 30 分钟、最多 2048 条。
